...
void print_usage(char* solver_name)
{
	printf("c Usage [1]: %s <instance> (Using default seed)\n", solver_name);
	printf("c Usage [2]: %s <instance> <seed> (Receiving seed from command line)\n", solver_name);
	fflush(stdout);
}
...
int main(int argc, char* argv[])
{
    ...
    //seed = time(0);
	if(argc==2) seed = 124;
	else if(argc==3) sscanf(argv[2],"%d",&seed);
	else seed = time(0);
	srand(seed);
